...
- `.eslintrc.cjs`: defines the global ESLint configuration.
...
- `jest.config.cjs`: defines the global Jest configuration which is inherited by each test suite.
- `tsconfig.json`: defines the global TypeScript configuration.
- `webpack-dts.config.cjs`: defines the type generation configuration used by webpack.
- `webpack-hmr.config.cjs`: defines the hot module replacement configuration used by webpack.
- `webpack.config.cjs`: defines the production configuration used by webpack.
- `webpack.util.cjs`: defines the global Webpack configuration used by each sub `webpack` config.
